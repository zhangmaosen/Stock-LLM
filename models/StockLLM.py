...
from transformers import LlamaConfig, LlamaModel, LlamaTokenizer, GPT2Config, GPT2Model, GPT2Tokenizer, BertConfig, \
    BertModel, BertTokenizer, AutoConfig, AutoTokenizer, AutoModelForCausalLM, Qwen2Model
...
class Model(nn.Module):
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        if configs.llm_model == 'Qwen':
            ...
            self.qwen_config.output_hidden_states = False
            try:
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, x_txts=None, y_txts=None, mask=None):
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            dec_out = self.forecast(x_enc, x_mark_enc, x_dec, x_mark_dec, x_txts, y_txts)
            return dec_out[:, -self.pred_len:, :]
        return None

    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, x_txts, y_txts):
        ...
        for b in range(x_enc.shape[0]):
            ...
            # )
            prompt_ = '<|start_prompt|>这是一只中国A股股票，请根据内容和日K线特征，对未来的收盘价进行预测：'
