...
for ii in range(args.itr):
    ...
    for epoch in range(args.train_epochs):
        ...
        for i, (batched_tensors, batched_df) in tqdm(enumerate(train_loader)):
            ...
            if (i + 1) % 100 == 0:
                accelerator.print(
                    "\titers: {0}, epoch: {1} | loss: {2:.7f} | average loss: {3:.7f}".format(i + 1, epoch + 1, loss.item(), np.average(train_loss)))
        ...
        vali_loss, vali_mae_loss = stock_vali(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric)
        test_loss, test_mae_loss = stock_vali(args, accelerator, model, test_data, test_loader, criterion, mae_metric)
        accelerator.print(
